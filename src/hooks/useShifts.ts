...
// API functions
const fetchShifts = async (params?: { date?: string; startDate?: string; endDate?: string }): Promise<ShiftAssignment[]> => {
...
const updateShift = async ({ id, data }: { id: string; data: UpdateShiftData }): Promise<ShiftAssignment> => {
    const response = await fetch(`/api/shifts/${id}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
