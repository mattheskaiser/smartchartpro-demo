...
export const ResidentConditionsMolecule = ({ 
  ...
  const addCondition = (conditionData: { name: string; diagnosedDate: string; status: string; notes: string }) => {
    if (editingCondition) {
  ...
  return (
    <CardAtom>
      <TextAtom variant="h3" weight="medium" className="mb-4">Medical Conditions</TextAtom>
          ...
          <div key={condition.id} className="p-4 bg-gray-50 rounded-md">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-1 rounded-full ${getStatusColor(condition.status)}`}>
                  <TextAtom variant="caption" as="span">{condition.status}</TextAtom>
                </span>
              </div>
              {isEditing && (
                <ActionMenuMolecule
            ...
            {condition.diagnosedDate && (
              <TextAtom variant="small" color="muted" className="mb-1">
                <TextAtom variant="small" weight="medium" color="muted" as="span">Diagnosed:</TextAtom> {new Date(condition.diagnosedDate).toLocaleDateString()}
              </TextAtom>
            )}
            {condition.notes && (
              <TextAtom variant="small" color="secondary">{condition.notes}</TextAtom>
            )}
          </div>
        ))}
        {conditions.length === 0 && (
